...
function formatData(data, options) {
  var templateString = '${log}'
  if (options.showInstanceName) {
    templateString = '(${instanceName}) ' + templateString
  }
  if (options.showTimestamp) {
    templateString = '[${timestamp}] ' + templateString
  }

  var logLineTmpl = _.template(templateString)

  var logs = data.map(function(d) {
    var timestamp = d._source.time.split('.')[0]
    var logLine = logLineTmpl({
      timestamp: timestamp,
      instanceName: d._source.mesos_task_id,
      log: d._source.log
    })
